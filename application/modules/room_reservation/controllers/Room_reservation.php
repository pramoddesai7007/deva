...
class Room_reservation extends MX_Controller {
    ...
    public function __construct()
    {
		...
		$this->load->model(array(
			...
			'room_facilities/roomcompanidetails_model'
		));	
    }
    public function bookingdatatable(){
   	...
   	//kiran chavan
   	public function adv_bookingdatatable(){ 
   		$today = date('Y-m-d');
		$params = $columns = $totalRecords = $data = array();
		$params = $_REQUEST;
	    $columns = array( 
		...
		if(!empty($params['search']['value']) ) {   
			//$where .=" WHERE ";
			$where .=" AND ( booked_info.booking_number LIKE '".$params['search']['value']."%' ";    
			$where .=" OR booked_info.full_guest_name LIKE '".$params['search']['value']."%' ";
			$where .=" OR booked_info.room_no LIKE '".$params['search']['value']."%' ";
			$where .=" OR customerinfo.firstname LIKE '".$params['search']['value']."%' ";
			$where .=" OR customerinfo.cust_phone LIKE '".$params['search']['value']."%' ";
			$where .=" OR booked_info.checkindate LIKE '".$params['search']['value']."%' ";
			$where .=" OR booked_info.checkoutdate LIKE '".$params['search']['value']."%' ";
			$where .=" AND booked_info.bookingstatus LIKE '0' )";

		}else{
			$where =" AND (booked_info.bookingstatus=0 )";
		}
		// getting total number records without any search
		$sql = "SELECT booked_info.*,customerinfo.firstname,customerinfo.cust_phone FROM booked_info Left join customerinfo ON customerinfo.customerid=booked_info.cutomerid WHERE 1 AND date(booked_info.checkindate) > '$today' AND booked_info.adv_booking = 1";
    ...
    public function checkindatatable(){
		...
		foreach($queryRecords as  $value){
			...
			if($this->permission->method('room_reservation','read')->access()):
			$customer_invoice='<a
			href="'.base_url().'room_reservation/customer-reciept/'.$value->bookedid.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="top" data-original-title="Customer Invoice" title="Customer Invoice"><i class="ti-receipt"></i></a>';
			endif;
			// end Vivek
			if($this->permission->method('room_reservation','read')->access()):
			$print='<input name="url" type="hidden" id="url_'.$value->bookedid.'"/><a onclick="printresrvation('.$value->bookedid.')" class="btn btn-primary btn-sm margin_right_5px" data-toggle="tooltip" data-placement="top" data-original-title="Print" title="Print"><i class="fa fa-print text-white" aria-hidden="true"></i></a>';
			...
			$datediff = strtotime($value->checkoutdate) - strtotime($value->checkindate);
            //$datediff = ceil($datediff/(60*60*24)); //kiran
            $datediff = $value->days?$value->days:ceil($datediff/(60*60*24));//kiran
			$totalPrice = $value->total_price*$datediff;
			if($value->bookingstatus==0){
				$status="Pending";
				}
			...
			$row[] =$status;
			$row[] =$paymentStatus;
			$row[] =$update.$checkin.$view.$customer_invoice.$print.$cancel;
			$data[] = $row;
    ...
    { 	// echo "Index"; die;
		...
		// $data["paylist"] = $this->roomreservation_model->edit_payments('12');
		// var_dump($data["paylist"]);die;
        #
        #pagination ends
        #   
        $data['module'] = "room_reservation";
        if($id){
        	$data['page']   = "adv_reservationlist";
        }else{
    ...
    public function booking($id = null)
    {
		...
		$data['companylist']   = $this->roomcompanidetails_model->company_dropdown();
		$data["gstlist"] = $this->db->select("*")->from("roomgstdetails")->get()->result();//kiran
		//var_dump($data["gstlist"]);die;
        #
        #pagination ends
        #   
        $data['module'] = "room_reservation";
    ...
    public function bookingedit($id = null)
    {
		...
		$data["setting"] = $this->db->select("servicecharge")->from("setting")->get()->row();
		$data['companylist']   = $this->roomcompanidetails_model->company_dropdown();
		$data["paylist"] = $this->roomreservation_model->edit_payments($id);
		$data["gstlist"] = $this->db->select("*")->from("roomgstdetails")->get()->result();//kiran
// var_dump($data["paylist"]);die;
        #
        #pagination ends
	...
	// vivek
	public function checkoutlist($id = null)
    {
		//echo "die list"; die;
		$this->permission->method('room_reservation','read')->redirect();
				
        $data['title']    = "Check Out List"; 
        #
        #pagination ends
        #   
	...
	public function submitcheckout($bookedid){
		...
		for($i=0; $i<count($bid); $i++){
			...
			if($result){
				...
				//sending whatsapp message to customer kiran..
				$appName = $this->db->select("title")->from("setting")->where("id",2)->get()->row();
				$kinfo = $this->db->select("b.booking_number,b.room_no,b.total_price,c.firstname,c.lastname,c.email,c.cust_phone,b.checkindate,b.checkoutdate")->from("booked_info b")->join("customerinfo c","c.customerid=b.cutomerid","left")->where("bookedid",$bid[0])->get()->row();
				$checkin  = date('dS F Y h:i a',strtotime($kinfo->checkindate));
				$checkout = date('dS F Y h:i a',strtotime($kinfo->checkoutdate));
				$msg_wp = "Dear $kinfo->firstname $kinfo->lastname,It was a pleasure hosting you at $appName->title.We trust that you are safely back at home. We would like to thank you for choosing us to spend your vacation.\n Thank you and we hope to see you again soon!";
				$whatsapp = $this->roomreservation_model->sendWhatsappSms($kinfo->cust_phone,$msg_wp,'');
	...
	public function checknewroom(){
		...
		$numberlist=$this->db->select("*")->from('tbl_roomnofloorassign')->where('roomid',$room_type)->where('status!=', 3)->where('status!=', 7)->get()->result();
		//->where('roomassignid!=',10)
		//var_dump($numberlist);die;
		$roomlist='';
	...
	public function getcapacity(){
		...
		//kiran
		if($settings->booking_type==0){
			$days = 0;
			$start_date_new =date('Y-m-d',strtotime($start));
			$end_date_new =date('Y-m-d',strtotime($end));
			$start_date = strtotime($start_date_new);
			$end_date = strtotime($end_date_new);
			$difference = $end_date - $start_date;
			$days =  ceil($difference / (60 * 60 * 24));

			$checkintime  = date('H:i:s',strtotime($start));
			$checkouttime = date('H:i:s',strtotime($end));
			$settings     = $this->db->select("*")->from("setting")->where("id",2)->get()->row();

			$d1 = strtotime($checkouttime);
			$d2 = strtotime($settings->checkouttime);
			$d3 = strtotime($checkintime);
		...
		//gst calculation
		$roomrate     = $capacity->rate * $days;
		$get_gst_rate = $this->db->select("*")->from("roomgstdetails")->where("'$roomrate' BETWEEN `fromgst` AND `togst`")->get()->row();
		 
		$gst_per = $get_gst_rate->gst?$get_gst_rate->gst:0;
		$gstamt  = ($roomrate * $get_gst_rate->gst)/100; 

		//end gst calculation
		// Add gst in price vivek
	   // $rate_with_gst = (($roomrate * $days)  + $gstamt);
		$data = array(
	...
	public function newBooking(){ 
		...
		$days = $this->input->post('days', TRUE)?$this->input->post('days', TRUE):0;
		...
		$gstamt       = $this->input->post('gstamt', TRUE);
		$total_gstamt = $this->input->post('total_gstamt', TRUE);

		//kiran chavan - for advance booking
		$currentdate = date('Y-m-d');
		$checkdate   = date('Y-m-d',strtotime($datefilter1));
		if(date($checkdate) > date($currentdate)){
		  $adv_booking = 1;
		}else{
		  $adv_booking = 0;
		}
		...
		// Vivek
		// $paymentmode = $this->input->post('paymentmode', TRUE);
		if(!empty($this->input->post('paymentmode_vv'))){
			$paymentmode = $this->input->post('paymentmode_vv', TRUE); 
		}else{
			$paymentmode = $this->input->post('paymentmode', TRUE);
		}
		...
		$allgst = explode(",",trim($gst));
		$allgstamt = explode(",",trim($gstamt));
		$totalprice=0;
		...
		//kiran chavan 
        $payableamt_k = $this->input->post("payableamt_k", true);
        $paymentmode_k = $this->input->post("paymentmode_k", true);
        $paymentamount_k = $this->input->post("paymentamount_k", true);
        $bankname_k = $this->input->post("bankname_k", true);
        $cardno_k = $this->input->post("cardno_k", true);
        $recdate_k = $this->input->post("recdate_k", true);
        //end kiran
		...
		if((!empty($customer[0])?$customer[0]:null)!=$allname[0] && !empty($allname[0])){
		   ...
		   $userdata = array(
			...
			'city' 		  => $this->input->post('city',TRUE),
			'zipcode' 	  => $this->input->post('zipcode',TRUE),
			'gst_no' => $this->input->post('gst_no',TRUE),//k
			'companyname' => $this->input->post('companyname',TRUE),//k
			'employee_id' => $this->input->post('employee_id',TRUE),//k
			'address' 	  => $this->input->post('address',TRUE),
		...
		//gst amount add in total price kiran
		//$grandtotal=($totalprice+$taxamount);
		// $grandtotal=($totalprice+$total_gstamt); // vivek
		...
		//end
			...
			if($this->roomreservation_model->create($postData)) { 
				...
				//save payment grid kiran
	            $singlepayment = explode(",", $paymentmode_k);
	            $singleamount = explode(",", $paymentamount_k);
	            $singlebankname = explode(",", $bankname_k);
	            $singlecardno = explode(",", $cardno_k);
	            $paymentdates = explode(",", $recdate_k);
	            ...
	            $postData = array(
	                ...
	                'details'       => "Card/Account No: ".$singlecardno[$i]." Bank Name: ".$singlebankname[$i],
	                'book_type'     => 0,
	                'card_acc'      => $singlecardno[$i],
	                'bank_name'     => $singlebankname[$i],
	                'payment_date'  => $paymentdates[$i],
	                );
	            $this->db->insert('tbl_checkinpayments', $postData);
	            }
	            //end save payment grid kiran
				...
				//end
        				...
        				//sending whatsapp message to customer kiran..
        		$appName = $this->db->select("title")->from("setting")->where("id",2)->get()->row();
        		$kinfo = $this->db->select("b.booking_number,b.room_no,b.total_price,c.firstname,c.lastname,c.email,c.cust_phone,b.checkindate,b.checkoutdate")->from("booked_info b")->join("customerinfo c","c.customerid=b.cutomerid","left")->where("bookedid",$bookedid)->get()->row();
        		$checkin  = date('dS F Y h:i a',strtotime($kinfo->checkindate));
        		$checkout = date('dS F Y h:i a',strtotime($kinfo->checkoutdate));
        		$msg_wp = "A very good day, $kinfo->firstname $kinfo->lastname. We confirm that you have a reservation at $appName->title from the $checkin to $checkout";
        		
        
        		//$this->newbooking_pdf($bid[0],'pdf');
        		$this->newbooking_pdf($bookedid,'pdf');
        		//$filename=APPPATH.'pdf/avd_book'.str_replace(' ', '_', $bookinfo->bookedid).'.pdf';
        		//$filename=APPPATH.'pdf/avd_book'.str_replace(' ', '_', $bookedid).'.pdf';
        		$filename=base_url().'application/pdf/avd_book'.str_replace(' ', '_', $bookedid).'.pdf';
        		
        		$whatsapp = $this->roomreservation_model->sendWhatsappSms($kinfo->cust_phone,$msg_wp,$filename);
        		//sending whatsapp message to customer kiran..
        		//end 
        		//var_dump($whatsapp);die;
				//end 
			...
			$days = $this->input->post('days', TRUE)?$this->input->post('days', TRUE):0;//kiran

			// $grandtotal=($totalprice+$total_gstamt)-$promocode; //kiran
			$grandtotal=($totalprice)-$promocode; //kiran
			...
			$this->roomreservation_model->delete_payment($bookingid);
            ...
            $hh = $this->db->insert('tbl_checkinpayments', $postData);
	...
	public function checkinBooking(){
		...
		// $grandtotal=($totalprice+$total_gstamt);  //......kiran
		$grandtotal=($totalprice); // vivek
		//end
			 $postData = array(
			   'booking_number' 	     => $bookingnumber,
			   'date_time' 	             => date('Y-m-d H:i:s'),
			...
			//sending whatsapp message to customer kiran..
			$appName = $this->db->select("title")->from("setting")->where("id",2)->get()->row();
			$kinfo = $this->db->select("b.booking_number,b.room_no,b.total_price,c.firstname,c.lastname,c.email,c.cust_phone,b.checkindate,b.checkoutdate")->from("booked_info b")->join("customerinfo c","c.customerid=b.cutomerid","left")->where("bookedid",$bookingid)->get()->row();
			$checkin  = date('dS F Y h:i a',strtotime($kinfo->checkindate));
			$checkout = date('dS F Y h:i a',strtotime($kinfo->checkoutdate));
			$msg_wp = "A very good day, $kinfo->firstname $kinfo->lastname. Your checkin Successfully at $appName->title from the $checkin to $checkout. Wishing you happy vacation.Enjoy and stay happy.";
			$whatsapp = $this->roomreservation_model->sendWhatsappSms($kinfo->cust_phone,$msg_wp,'');
			//var_dump($whatsapp);die;
			//sending whatsapp message to customer kiran..
			//end
 ...
 public function addpayment($bid){
	...
	public function room_status($id = null)
    {
		...
		$this->db->where('tbl_roomnofloorassign.status!=',7);
		$this->db->where('tbl_roomnofloorassign.status!=',3);
        ...
        $data["bookings"] = $this->roomreservation_model->get_booking_det(); 
        $data["guest_room"] = $this->roomreservation_model->get_booking_guest_room_det();
	...
	public function changeStatus(){
		...
		$room_status = $this->input->post("room_status", true);
		$result= 1;
		$this->db->set('tbl_roomnofloorassign.status',$room_status);
		$this->db->where('tbl_roomnofloorassign.roomassignid =',$roomid);
		$result = $this->db->update('tbl_roomnofloorassign');
	...
	public function newbooking_pdf($id,$pdf=null){ 
		$details=$this->roomreservation_model->details($id);
		$data['bookinfo']   = $details;
		$data['customerinfo']   = $this->roomreservation_model->customerinfo($details->cutomerid);
		$data['paymentinfo']   = $this->roomreservation_model->paymentinfo($details->bookedid);
		...
		if(empty($pdf)){
			...
			$pdfhtml = $this->load->view('room_reservation/advance_pdf', $data);
			//var_dump($pdfhtml);die;
			//return $pdfhtml;
		}
	}
	...
	public function undo_checkout(){ 
		$bookingid      = $this->input->post("bookingid", true);
		$booked_info = $this->db->select("*")->from("booked_info")->where("bookedid",$bookingid)->get()->row();
		$booked_details = $this->db->select("*")->from("booked_details")->where("bookedid",$bookingid)->get()->row();
		
		$tbl_guestpayments_vchno = $this->db->select("*")->from("tbl_guestpayments")->where("bookedid",$bookingid)->where("`details` NOT LIKE 'Advance%' ")->get()->result();
	
        
		$arr = [];
		foreach ($tbl_guestpayments_vchno as $item) {
			$arr[] = $item->invoice;
		}
		
		$tbl_guestpayments_vchno_k = implode(',',$arr);
		
		$this->db->where_in('VNo',$tbl_guestpayments_vchno_k);
        $this->db->delete('acc_transaction');

		$this->db->where_in('invoice',$tbl_guestpayments_vchno_k);
        $rs=$this->db->delete('tbl_guestpayments');

		$this->db->set('booked_info.payment_mode',$booked_details->payment_method); // vivek
		$this->db->set('booked_info.paid_amount',$booked_details->advance_amount);
		$this->db->set('booked_info.bookingstatus','4');
		$this->db->where('booked_info.bookedid =',$bookingid);
		$result = $this->db->update('booked_info');
		
		if($result){
			$msg = '<h5>Undo Checkout</h5> Successful'; 
			$data["msg"] = $msg;
			$data["result"] = 1;
			echo json_encode($data);
		}else{
	...
	public function checkin_reminder(){
		$recent_time = date('Y-m-d H:i'); 
		$recent_day = date('Y-m-d'); 
 		$tommarow_date = date('Y-m-d',strtotime('+1 day',strtotime($recent_day)));
 		$data['reminder_info']=$this->roomreservation_model->detailbooking_forremider(array('checkindate'=>$tommarow_date));
		echo $data['reminder_info'];die;
	...
	public function customer_receit($id){
		...
		$data['customerinfo']   = $this->roomreservation_model->customerinfo($details->cutomerid);
		$data['paymentinfo']   = $this->roomreservation_model->paymentinfo($details->bookedid);
		$data['commoninfo']=$this->roomreservation_model->commoninfo();
		$data['storeinfo']=$this->roomreservation_model->storeinfo();
		$data['module'] = "room_reservation";
	...
	// Vivek	
	public function checkoutlistdatatable(){
		$params = $columns = $totalRecords = $data = array();
		$params = $_REQUEST;
	    $columns = array( 
		0 => 'booked_info.bookedid', 
		...
		// getting total number records without any search
		$sql = "SELECT booked_info.*,customerinfo.firstname,customerinfo.cust_phone FROM booked_info Left join customerinfo ON customerinfo.customerid=booked_info.cutomerid where booked_info.bookingstatus=5";
		...
		foreach($queryRecords as  $value){
			...
			$row[] =$status;
			$row[] =$paymentStatus;
			$row[] =$view.$customer_invoice.$print;
			$data[] = $row;
