...
$(document).ready(function() {
    ...
    $("#rent").on("change", function(){ //make same changes in calculate_total fun both fun same
        var all = $("table.room-list > tbody").length
        var rent = parseFloat($("#rent").val()); 
...
var newRoom = $("<tbody id='myTableBody'>");
var col = '<tr><th colspan="2">Room Info<span style="padding-left: 30%;"></span>Complementery <input type="checkbox" name="complementary_checkbox" id="complementary_checkboxt'+i+'" readonly="true" value="1" onclick="getcapcitys('+i+')"></th><th>Action</th></tr>';
col += '<tr><td colspan="2"><table class="table table-borderless mb-0"><tbody><tr>'
       ...
       +'<!--td class="border-0">'
       +'<div class="form-group mb-0"><label class="font-weight-600 mb-1">#gst</label><div class="icon-addon addon-md input-left-icon"><input type="number" min="1" readonly="true" class="form-control"'
       ...
       +'id="gstamt'+i+'" placeholder="gstamt" value="0"><label class="fas fa-tags"></label></div></div></td></tr></tbody></table></td><td rowspan="3" class="text-center"><button type="button" class="btn btn-danger btn-sm rdel'+r+'" onclick="rdel('+r+')"><i class="far fa-trash-alt"></i></button>'
       +'</td></tr>';
...
col += '<tr><td><table class="table table-borderless customerdetail'+i+'"><thead><tr><th class="pl-0" width="20">Pri?</th><th>Name</th><th>Mobile No.</th>'
       ...
       +'<td class="border-0"><div class="form-group mb-0"><label class="font-weight-600 mb-1">Rent</label><div class="icon-addon addon-md"><input type="number" disabled class="form-control form-control" onchange="getgstamt('+r+')" id="rent'+i+'"'
...
$.ajax({
    ...
    success: function(data) {
        ...
        if(complementary_checkboxt==1){
            ...
            $("#rent"+n).val(0);
            //kiran
            $("#gst"+n).val(obj.gst); // uncoment vivek
            $("#gstamt"+n).val(obj.gstamt); // uncoment vivek
            //kiran
...
function calculate_total(){ //make same changes in rate change fun both fun same
        var all = $("table.room-list > tbody").length;
        var rent = parseFloat($("#rent").val());
        var tax = parseFloat($("#tax_percent").val());
        var scharge = parseFloat($("#service_percent").val());
