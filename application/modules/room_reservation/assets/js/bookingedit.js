...
$(document).ready(function() {
    ...
    $("#rent-1").on("change", function(){  //make same changes in calculate_total fun both fun same
        var all = $("table.room-list > tbody").length
        var rent = parseFloat($("#rent-1").val());
...
function room(r,s=null){
    ...
    var gstlist = $("#gstlist").html();
    var newRoom = $("<tbody>");
    var col = "";
    col = '<tr><th colspan="2">Room Info<span style="padding-left: 30%;"></span>Complementery <input type="checkbox" name="complementary_checkbox" id="complementary_checkboxt'+i+'" readonly="true" value="1" onclick="getcapcitys('+i+')"></th><th>Action</th></tr>';
    col += '<tr><td colspan="2"><table class="table table-borderless mb-0"><tbody><tr>'
           ...
           +'</div></div></td>'
           +'<td class="border-0"><div class="form-group mb-0"><label class="font-weight-600 mb-1">#Gst</label>'
           +'<div class="icon-addon addon-md input-left-icon"><select class="selectpicker form-select"data-live-search="true" data-width="100%" onchange="getgstamt('+i+')" id="gst'+i+'">'
           +'<option value="" selected>Choose Room Type</option>'+gstlist+'</select><label class="fas fa-sort-amount-down"></label></div></div></td>'
           +'<!--td class="border-0"><div class="form-group mb-0"><label class="font-weight-600 mb-1">#gst</label><div class="icon-addon addon-md input-left-icon"><input type="number" min="1" readonly="true" class="form-control"'
           +'id="gst'+i+'" placeholder="gst"><label class="fas fa-percent"></label></div></div></td--><td class="border-0">'
           +'<div class="form-group mb-0"><label class="font-weight-600 mb-1">#gstamt</label><div class="icon-addon addon-md input-left-icon"><input type="number" min="1" readonly="true" class="form-control"'
           +'id="gstamt'+i+'" placeholder="gstamt" value="0"><label class="fas fa-tags"></label></div></div></td></tr></tbody></table></td><td rowspan="3" class="text-center"><button type="button" class="btn btn-danger btn-sm rdel'+i+'" onclick="rdel('+i+')"><i class="far fa-trash-alt"></i></button>'
           +'</td></tr>';
    ...
    col += '<tr><td><table class="table table-borderless customerdetail'+i+'"><thead><tr><th class="pl-0" width="20">Pri?</th><th>Name</th><th>Mobile No.</th>'
           ...
           +'<td class="border-0"><div class="form-group mb-0"><label class="font-weight-600 mb-1">Rent</label><div class="icon-addon addon-md"><input type="number" onchange="getgstamt('+i+');" class="form-control form-control" id="rent'+i+'"'
...
function getroomnos(n){
    ...
    var end1 = $("#datefilter2").val();
    var split = end1.split(' ');
    var splitT = outtime.split(' ');
    var datefilter2 = split[0] +" "+splitT[1];
    //alert(datefilter2);
   //var datefilter2 = $("#datefilter2").val(datefilter2);
    // end vivek
...
function getcapcitys(n){ 
    ...
    var room_type= $("#room_type"+n).find(":selected").val();
    var complementary_checkboxt = $('#complementary_checkboxt'+n+':checked').val()?$('#complementary_checkboxt'+n+':checked').val():0;
    if(roomno==""){
        if(room_type!=""){
        $("#roomno"+n).removeClass("is-valid");
        $("#roomno"+n).closest('div').removeClass("is-valid");
    ...
    var end = $("#datefilter2").val(); // vivek
    // var end = $("#datefilter2").val(end2);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(complementary_checkboxt==1){
                ...
                //kiran
                $("#gst"+n).val(obj.gst); // vivek Uncoment
                $("#gstamt"+n).val(obj.gstamt); // vivek Uncoment
                //kiran
...
function calculate_total(){ //make same changes in rate-1 change fun both fun same
        ...
        //var total = rent+tax+scharge; //kiran
        // var total = rent+gstamt+scharge; // vivek
        var total = rent+scharge;
        $("#totalamount").val(total);
        //$("#tax_charge").text(tax); //kiran..
        $("#booking_charge").text(rent); //kiran
        $("#tax_charge").text(gstamt);
